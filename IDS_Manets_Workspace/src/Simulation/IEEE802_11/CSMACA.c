...
static double get_CSMACA_Time(PIEEE802_11_MAC_VAR mac, PIEEE802_11_PHY_VAR phy)
{
	if (!phy->dControlFrameDataRate)
		fnNetSimError("Control frame data rate is 0 for device %d, interface %d in %s\n",pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId,__FUNCTION__);
	return phy->DIFS +phy->plmeCharacteristics.aSIFSTime +get_preamble_time(phy) +((getAckSize(phy) * 8) / phy->dControlFrameDataRate);
